...
stngs = strings(6,1);stngs(1) = ':k';stngs(2) = '-.k';stngs(3) = '--k';stngs(4) = '-k';
...
while t < T
	...
	fplus = (((3/2)*U0(1) - (1/2)*U0(N))^2)/2;
	fminus = (((3/2)*U0(1) - (1/2)*U0(2))^2)/2;
	U(1) = U0(1) - (k/h)*(fplus - fminus);
	
	% Copmute intermediate points
	for j = 2:N-1
		fplus = (((3/2)*U0(j) - (1/2)*U0(j-1))^2)/2;
		fminus = (((3/2)*U0(j) - (1/2)*U0(j+1))^2)/2;
		U(j) = U0(j) - (k/h)*(fplus - fminus);
	end
	
	% Compute right endpoint
	fplus = (((3/2)*U0(N) - (1/2)*U0(N-1))^2)/2;
	fminus = (((3/2)*U0(N) - (1/2)*U0(1))^2)/2;
...
xlabel('x')
ylabel('u(x)')
title('2nd Order(Method 2) Solution Over Time')
hold on
