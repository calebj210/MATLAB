...
% Initial error plot at each node (Everything is zero so that's good)
% plot(x(1:end-1),u(3:N+2) - -(cos(x(2:end)) - cos(x(1:end-1)))/h)
% pause(0.5)

% Begin times stepping
for t = 1:M
	for i = 3:N+2
...
trU =-(cos(x(2:end) - t*k) - cos(x(1:end-1) - t*k))/h;
