...
N = 100; % Number of nodes on curve
...
tmp = nodes(2,:)-nodes(N,:);
scal = norm(tmp);
calcNorms(1,1) = tmp(2)./scal;
calcNorms(1,2) = -tmp(1)./scal;

% Middle Nodes %
for i = 2:N-1
	tmp = nodes(i+1,:)-nodes(i-1,:);
	scal = norm(tmp);
	calcNorms(i,1) = tmp(2)./scal;
	calcNorms(i,2) = -tmp(1)./scal;
end

% Last Node %
tmp = nodes(1,:)-nodes(N-1,:);
scal = norm(tmp);
calcNorms(N,1) = tmp(2)./scal;
calcNorms(N,2) = -tmp(1)./scal;
