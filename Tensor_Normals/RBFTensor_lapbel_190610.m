...
%if fig = 3 Display error line plots
%if fig = 4 DISPLAY GLOBAL FIGURES (laplace beltromi error at each node)
%if fig = 5 DISPLAY LOCAL FIGURES (interpolation of function over surface)
%if fig = 0  NO FIGURES
% fig = 1;
fig = 4;
...
%% Function on Surface
%Parametic Form
func =@(t) sin(2.*pi.*sin(t));
%unit circle trulapbelfunc
trulapbelfunc =@(t) -2.*pi.*(cos(2.*pi.*sin(t)).*sin(t) + 2.*pi.*cos(t).^2.*sin(2.*pi.*sin(t))); %unit circle
...
errlapbel = zeros(2*size(nodedist,1),maxpolydeg-minpolydeg+1);
...
%    load (fullfile('Equispaced_nodes_pi',[fileName '.mat']));   %pi curve
    ...
    for polydeg = minpolydeg:maxpolydeg  %loop over adding neighbors by adding polynomial terms
        ...
        calctangs = zeros(N,2);
        trutangs = zeros(N,2);
        calclapbel = zeros(N,1);
        S1D_func = sparse(N,N);
        S1D_gamma = sparse(N,N);
...
%         phi1D =@(X1,X2,m) sqrt((X1-X2).^2).^m;
       ...
       phi1D_Dx = @(X1,X2,m) sign(X1-X2).*m.*((abs(X1-X2)).^(m-1));
...
%        X(1:N,1:2) = vpa(gamma(1:N,:),precision);
         X(1:N,1:2) = gamma(1:N,:);
         X(1:N,3) = func(t(1:N));
%        initnorms = vpa(zeros(N,2),precision);  %storage for initial normal guess, for background nodes
        initnorms = zeros(N,2);
        lapbel_DM = sparse(N,N);
%computing initnorms
...
%             end
            ...
            A11_gamma = phi1D(X1,X2,m);
            A12_gamma = (xn1(:,ones(1,p+1)).^x1D_exp(ones(1,n),:));
            
            Ax11_gamma = phi1D_Dx(X1,X2,m);
            Ax12_gamma = (xn1(:,ones(1,p+1)).^x1D_exp_Dx(ones(1,n),:));
            
            Axx11_gamma = phi1D_Dxx(X1,X2,m);
            Axx12_gamma = (xn1(:,ones(1,p+1)).^x1D_exp_Dxx(ones(1,n),:));
    
            B11_gamma = isites(2,:);
            B12_gamma = x1D_exp.*(xn1(1,ones(1,p+1)).^x1D_exp);
            
            B11_func = X(idx(index,:),3)'- (X(idx(index,1),3)*ones(1,length(isites)));
            B12_func = x1D_exp.*(xn1(1,ones(1,p+1)).^x1D_exp);
                       
%             Bx11 = phi1Dx(X1(1,:),X2(1,:),m);
%             Bx12 = x1D_exp.*(xn(1,ones(1,p+1)).^x1D_exp_Dx);
            
            B_gamma = [B11_gamma B12_gamma];
            B_func = [B11_func B12_func];
            
%            Bx = [Bx11 Bx12];
 
            A_gamma = [A11_gamma A12_gamma;A12_gamma' zeros(p+1)];
            
          Ax_gamma = [Ax11_gamma Ax12_gamma;Ax12_gamma' zeros(p+1)];
          Axx_gamma = [Axx11_gamma Axx12_gamma;Axx12_gamma' zeros(p+1)];

            S1D_localgamma = B_gamma/A_gamma; %local curve interpolation weights
            S1D_localfunc = B_func/A_gamma; %local overlayed function interpolation weights
 
%            S1D_gamma(index,idx(index,:)) = S1D_localgamma(1,1:n);        
  %          S1D_func(index,idx(index,:)) = S1D_localfunc(1,1:n);
            if fig ==2
                Xinterp_gamma = linspace(min(isites(1,:)),max(isites(1,:)),2*n)';
                [Xinterp_gamma1, Xinterp_gamma2] = meshgrid(isites(1,:),Xinterp_gamma);
                Ainterp11_gamma = phi1D(Xinterp_gamma1,Xinterp_gamma2,m);
                Ainterp12_gamma = (Xinterp_gamma(:,ones(1,p+1)).^x1D_exp(ones(1,2*n),:));
                Ainterp_gamma = [Ainterp11_gamma Ainterp12_gamma];
                lambda_gamma = S1D_localgamma';
                Yinterp_gamma = Ainterp_gamma*lambda_gamma;
                figure
                hold on
                axis equal
                title('local RBF interpolation')
                plot(isites(1,:), isites(2,:), 'ko')
                plot(Xinterp_gamma,Yinterp_gamma,'b-')
                pause
            end
            
            if fig ==5
%                Xinterp_func = linspace(min(isites(1,:)),max(isites(2,:)),2*n)';
                Xinterp_func = linspace(min(isites(1,:)),max(isites(1,:)),2*n)';
                [Xinterp_func1, Xinterp_func2] = meshgrid(isites(1,:),Xinterp_func);
                Ainterp11_func = phi1D(Xinterp_func1,Xinterp_func2,m);
                Ainterp12_func = (Xinterp_func(:,ones(1,p+1)).^x1D_exp(ones(1,2*n),:));
                Ainterp_func = [Ainterp11_func Ainterp12_func];
                lambda_func = S1D_localfunc';
                Yinterp_func = Ainterp_func*lambda_func;
                figure
                hold on
                axis equal
                title('local RBF interpolation of laplace betlromi')
                plot(isites(1,:),X(idx(index,:),3)'- (X(idx(index,1),3)*ones(1,length(isites))), 'ko')
                plot(Xinterp_func,Yinterp_func,'b-')
                pause
            end
            
            % Compute local norm direction
            localnorm = Ax_gamma*S1D_localgamma';
  ...
  pause     
           ...
           % calcnorms(index,2) = revchangemat(2,1)*isites(1,1) + revchangemat(2,2)*(isites(2,1)+localnorm(1,1));
            ...
            calcnorms(index,1) = revchangemat(1,1)*(isites(1,1)-localnorm(1,1)) + revchangemat(1,2)*(isites(2,1)+1);
            calcnorms(index,2) = revchangemat(2,1)*(isites(1,1)-localnorm(1,1)) + revchangemat(2,2)*(isites(2,1)+1);

 %           localnorm(1,1)
 
 %%% Calculate Laplace Beltromi Operator

%      phigammaDX = phi1D_Dx(isites(1,:)',zeros(n,1),m);
%      phigammaDXX = phi1D_Dxx(isites(1,:)',zeros(n,1),m);
%      phifuncDX = phi1D_Dx(isites(1,:)',zeros(n,1),m);
%      phifuncDXX = phi1D_Dxx(isites(1,:)',zeros(n,1),m);

 
     phigammaDX = Ax_gamma(1,:);
     phigammaDXX = Axx_gamma(1,:);
     phifuncDX = Ax_gamma(1,:);
     phifuncDXX = Axx_gamma(1,:);

        funcX = S1D_localfunc(1,:)*phifuncDX';
        funcXX = S1D_localfunc(1,:)*phifuncDXX';
        gammaX =  S1D_localgamma(1,:)*phigammaDX';
        gammaXX = S1D_localgamma(1,:)*phigammaDXX';

     calclapbel(index,1)  = funcXX/(1+gammaX^2) -  ((funcX*gammaX*gammaXX)/((1+gammaX^2)^2));

 end
%% Normalizing the normal vectors at each node
scal = sqrt(calcnorms(:,1).^2 + calcnorms(:,2).^2); %normalizing contant
...
%     gammaX = 0;
%     gammaXX = 0;
%     Xgamma = X(i,1).*ones(N,1);
%     Xfunc  = X(i,1).*ones(N,1);
%     phigammaDX = phi1D_Dx(X(1:N,1),Xgamma,m);
%     phigammaDXX = phi1D_Dxx(X(1:N,1),Xgamma,m);
%     phifuncDX = phi1D_Dx(X(1:N,1),Xfunc,m);
%     phifuncDXX = phi1D_Dxx(X(1:N,1),Xfunc,m);
%     for j = 1:N
%        funcX = funcX + S1D_func(i,j)*phifuncDX(j,1);
%        funcXX = funcXX + S1D_func(i,j)*phifuncDXX(j,1);
%        gammaX =  gammaX + S1D_gamma(i,j)*phigammaDX(j,1);
%        gammaXX = gammaXX + S1D_gamma(i,j)*phigammaDXX(j,1);
%     end
%     calclapbel(i,1)  = funcXX/(1+gammaX^2) -  ((funcX*gammaX*gammaXX)/((1+gammaX^2)^2));
% end
...
trulapbel = zeros(N,1);
t = linspace(0,2*pi,N+1)';
trulapbel(:,1) = trulapbelfunc(t(1:N));


 
 %% Compute relative normal error
errnlapbel = abs(calclapbel -trulapbel);
...
axis equal
sz = 25;
scatter(X(1:N,1),X(1:N,2),sz,log10(errnlapbel),'filled')

end

%% Store labpbel error
errlapbel(nodes, polydeg- (minpolydeg - 1)) = size(X,1);
errlapbel(nodes +  size(nodedist,1),polydeg-(minpolydeg -1)) = norm(errnlapbel,inf);
...
if fig == 3
    ...
    %% Error Order line
    errorder1 = 3; %order of error
    c1 = 10; %appropriate constant to move error line to desired location
    errlin1 = zeros(length(nodedist),2);
    errorder2 = 5; %order of error
    c2 = 10; %appropriate constant to move error line to desired location
    errlin2 = zeros(length(nodedist),2);
    errorder3 = 7; %order of error
    c3 = 15; %appropriate constant to move error line to desired location
    errlin3 = zeros(length(nodedist),2);
    errorder4 = -2; %order of error
    c4 = -50; %appropriate constant to move error line to desired location
    errlin4 = zeros(length(nodedist),2);
    
    for i = 1:length(nodedist)
        errlin1(i,1) = errnorml2(i,1);
        errlin1(i,2) =  (nodedist(i).^(-errorder1)).*exp(c1);
        errlin2(i,1) = errnorml2(i,1);
        errlin2(i,2) =  (nodedist(i).^(-errorder2)).*exp(c2);
        errlin3(i,1) = errnorml2(i,1);
        errlin3(i,2) =  (nodedist(i).^(-errorder3)).*exp(c3);
        errlin4(i,1) = errnorml2(i,1);
        errlin4(i,2) =  (nodedist(i).^(-errorder4)).*exp(c4);
    end
%plot error of lapbel UNIT CIRCLE
figure
hold on
plot((log10(errlapbel(1:nodes,1))),(log10(errlapbel((nodes+1):2*nodes,1))),'b-o')
plot((log10(errlapbel(1:nodes,2))),(log10(errlapbel((nodes+1):2*nodes,2))),'k-o')
plot((log10(errlapbel(1:nodes,3))),(log10(errlapbel((nodes+1):2*nodes,3))),'g-o')
plot((log10(errlapbel(1:nodes,4))),(log10(errlapbel((nodes+1):2*nodes,4))),'r-o')
plot((log10(errlapbel(1:nodes,5))),(log10(errlapbel((nodes+1):2*nodes,5))),'c-o')
plot((log10(errlapbel(1:nodes,6))),(log10(errlapbel((nodes+1):2*nodes,6))),'m-o')
%plot(log10(errlin1(:,1)),log10(errlin1(:,2)),'--')
%plot(log10(errlin2(:,1)),log10(errlin2(:,2)),':')
%plot(log10(errlin3(:,1)),log10(errlin3(:,2)),'-.')
plot(log10(errlin4(:,1)),log10(errlin4(:,2)))
title('lapbel error -- Increasing Neighbors and Polynomial Terms')
xlabel('Log10(N)')
ylabel('Log10(err)')
legend({'n = 6', 'n = 8', 'n = 10', 'n = 12', 'n=14','n=16', 'O(h^3)','O(h^5)','O(h^7)','O(h^(-2))'},'location','southwest')


% %plot l2 error of normal PI
% figure
% hold on
...
% ylabel('Log10(err)')
% %legend({'n = 6', 'n = 8', 'n = 10', 'n = 12', 'n=14','n=16', 'O(h^3)','O(h^5)','O(h^7)','O(h^(-2))'},'location','southwest')
% legend({'n = 6', 'n = 8', 'n = 10', 'n = 12', 'n=14','n=16','O(h^(-2))'},'location','southwest')
% 
